...
namespace MPL::LumaService
{
    ...
    bool LumaService::RegisterClient(const MPL::API::Luma::ClientCallbacks* a_callbacks)
    {
        {
    ...
    std::vector<RegisteredClient> LumaService::GetCallbacks()
    {
        std::scoped_lock lock(callbackLock);
        return callbacks;
    };
        ...
        Callback&& a_callback)
    {
        try
        {
            for (const auto& client : LumaService::GetSingleton()->GetCallbacks())
            {
                try
                {
                    a_callback(client);
